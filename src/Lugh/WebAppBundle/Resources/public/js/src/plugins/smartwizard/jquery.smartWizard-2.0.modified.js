    ...
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                function init(){
                  ...
                  btNext = $('<a>'+options.labelNext+'</a>').attr("href","#").addClass("btn btn-primary buttonNext");
                  btPrevious = $('<a><i class="fa fa-angle-left"></i>'+options.labelPrevious+'</a>').attr("href","#").addClass("btn btn-primary buttonPrevious");
                  btFinish = $('<a>'+options.labelFinish+'<i class="fa fa-angle-right"></i></a>').attr("href","#").addClass("btn btn-primary btn-direction buttonFinish");
                ...
                function realHeight(obj){
                    ...
                    var element = $("div[id^='steps-uid-']").filter(":visible").filter("div[role='tabpanel']");
                    
                    element.find('.panel-body').append(clone);
                    var height = clone.outerHeight();
                    if (height === 0 && $("div[id^='step-']").length > 0)
                    {
                        //height = $('#step-1').outerHeight();
                        height =_.reduce($("div[id^='step-']"), function(height, ele) {if ($(ele).outerHeight() > 0) return $(ele).outerHeight(); return height;},0);
                    }
                    clone.remove();
                    return height;
                }
                ...
                function showStep(stepIdx){
                    ...
                    if ($($(selStep, obj).attr("href"), obj).outerHeight() != 0)
                    {
                        elmStepContainer.height($($(selStep, obj).attr("href"), obj).outerHeight());
                    }
                    else
                    {
                        elmStepContainer.height(realHeight($($(selStep, obj).attr("href"), obj)));  
                    }             
