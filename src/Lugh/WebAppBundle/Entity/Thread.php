...
class Thread extends Item
{
    ...
    /**
     * @ORM\ManyToOne(targetEntity="Accionista", inversedBy="threads", cascade={"persist"})
    ...
    public function setLockedState($locked)
    {
        if ($locked == null)
        {
            $this->lockedState = $this->getContainer()->get('lugh.server')->getState()->getUnlockedState();
        }
        switch ($this->getLocked()) {
            case StateClass::locked:
                $this->lockedState = $this->getContainer()->get('lugh.server')->getState()->getLockedState();
                break;
            case StateClass::unLocked:
                $this->lockedState = $this->getContainer()->get('lugh.server')->getState()->getUnlockedState();
                break;
            default:
                break;
    ...
    public function __construct() {
        ...
        $this->setLockedState($this->getLocked());
    }
